document.addEventListener("DOMContentLoaded", function() {
  ...
  if (analysisResult) {
    if (analysisResult.includes("Image uploaded does not seem to contain a medicine package or label. Please try again")) {
      errorMessageElement.textContent = "The uploaded image does not seem to contain a medicine package or label. Please try again.";
  ...
  fetch(`/api/chatbot/recognition-history/${patientID}`)
    ...
    .then(data => {
      if (data.status === "Success") {
        ...
        if (history.length === 0) {
          ...
          noHistoryMessage.innerHTML = '<td colspan="6" class="text-center">No history available.</td>';
    ...
    .catch(error => {
      ...
      errorMessageElement.textContent = 'An error occurred while fetching history.';
      errorMessageElement.style.display = 'block';
    });
});
