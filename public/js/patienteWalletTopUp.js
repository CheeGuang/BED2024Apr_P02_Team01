async function loadBalance() {
  ...
  try {
      const response = await fetch(`${window.location.origin}/api/patient/${patientId}/e-wallet-amount`);
      if (response.ok) {
          const data = await response.json();
          console.log(`E-wallet amount retrieved: ${data.eWalletAmount}`);
          currentBalanceElement.textContent = 'S$' + parseFloat(data.eWalletAmount).toFixed(2);

          // Update local storage balance
          localStorage.setItem('eWalletBalance', parseFloat(data.eWalletAmount).toFixed(2));
      } else {
...
async function updateBalanceFromQR() {
  ...
  if (patientId && !isNaN(topUpAmount)) {
      try {
          const response = await fetch(`${window.location.origin}/api/patient/topup/${patientId}`, {
              method: 'PUT',
              headers: {
          ...
          if (response.ok) {
              document.getElementById('current-balance').innerText = `S$${result.eWalletAmount.toFixed(2)}`;
